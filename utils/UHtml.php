	...
	*/
	public static function transformInternalLinks($content,$routes){
		return preg_replace_callback('#<a([^>]+data\-role="internalLink"[^>]*)>#U',function(&$m) use(&$routes){
			...
			return '<a href='.HHtml::url(call_user_func_array($routes[$type[1]],json_decode(html_entity_decode($params[1],ENT_QUOTES,'UTF-8'),true)),false,true).'" '.trim(preg_replace('#\s*(?:data\-role|data\-type|data\-params|href)="[^"]+"\s*#U',' ',$m[1])).'>';
		},$content);
	}
