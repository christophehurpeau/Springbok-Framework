...
*/		$recursifPattern='[^()]*(?:\([^()]+\)[^()]*)*';
		...
		$newPhpContent=preg_replace_callback('/((?:\$([^={}\(\)]+)\s*=\s*|\s+(?:self::)?set(?:ForLayout)?\([^,]+,|=>|if\(!?|&&\s*!?|\|\|\s*!?|foreach\(|implode\(\'[^\']+\',|return|else|;|}|((?:(?:CTable(?:One)?|CPagination[^\:]*)\:\:create|\->query)\(\s*)?\n)\s*'
				...
				.'\:\:(?:ById|ByIdAndStatus|ByIdAndType|QCount|QDeleteAll|QDeleteOne|QExist|QAll|QListValue|QListValues|QOne|QValue|QValues|QInsert|QInsertSelect|QLoadData|QReplace|QUnion|QUpdate|QUpdateOne|QUpdateOneField|QRow|QRows)\([^()]*(?:\([^()]+\)[^()]*)*\)'
