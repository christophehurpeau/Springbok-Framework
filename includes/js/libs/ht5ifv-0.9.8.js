 ...
 *version 0.9.8 (21-12-2011)
 *Add support for controls outside the form elements (e.g. <input .... form="formid"/>...<form id="formid">...</form>)
 */
...
(function($){
	...
	var $defaults = {
		...
		restrictions:{
			...
			'data-same':function($node){
				return $node.val() === $($node.data('same')).val();
			},
			'data-min-length':function($node,$ignoreEmpty){
				var $v = $node.val();
				return ($v =='' && $ignoreEmpty ) || $v.length >= $node.data('minLength')        
			}	
		},
		...
		init: function($o){
			...
			return this.each(function(){
				...
				var $controls = $options.filter(
					(function(){
						if ($form.is('input,textarea,select')){ //if it is already a control just return it
							return $form;
						}else{ 
							var $set1 = $('input,textarea,select',$form);  //search for controls which descendant of this $form element
							if($form.is('form[id]')){ //search for inputs of this form (with same name and type) elsewhere in document
								var $id = $form.attr('id');
								var $set2 = $('input,textarea,select').filter(function(){
									var $this = $(this);
									var $forms = $this.attr('form');
									if ($forms === undefined) return false;
									return (function($a){ //don't break this line after the return
			...
			/*	$checkboxes.each(function(){ 
				...
				var $htmlForm = (function(){ //find the form element of 
					return $form.is('form') ? $form : (function(){
						var $f = $form.parents('form').first();
						return $f.is('form') ? $f : $('body'); //use elerment body as the last resource
					})()
				})()
					...
					$.each($restrictions,function($restriction,$evaluate){
						if($self.is('[' + $restriction + ']')){
							...
							var $radioGroup = (function(){
								...
								if (!$name) return $self;
								var $type = $self.attr('type');
								var $group = $htmlForm.find('input[type]').filter(function(){ //search for other inputs of same name (and type)
									var $this = $(this);
									return $type == $this.attr('type') && $name === $this.attr('name')
								});
								if($htmlForm.is('form[id]')){ //search for inputs of this form (with same name and type) elsewhere in document
									var $id = $htmlForm.attr('id');
									var $group2 = $('input').filter(function(){
										var $this = $(this);
										var $forms = $this.attr('form');
										if ($forms === undefined) return false;
										return (function ($a){
